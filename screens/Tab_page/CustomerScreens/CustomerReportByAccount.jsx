...
const CustomerReportByAccount = ({ navigation, route }) => {
  const { CustomerData } = route.params;
  const customerData = CustomerData;

  const renderAmountRow = (amount, customerName) => (
  ...
  const renderCustomerRows = (customer) => (
    <>
      {customer.amount.map((amount) => {
        const formattedDateea = moment(amount.time).format('D MMM YYYY');
        return (
        <View className="flex-row  mx-2 my-1 py-1 bg-gray-400 rounded-lg ">
          <View className="w-[50%]">
            <Text className="text-md text-black pl-2 font-bold">{customer.name}</Text>
  ...
  const filterDataByMonth = (data, selectedMonth) => {
    return data.filter((customer) =>
      customer.amount.some(
        (amount) => new Date(amount.time).getMonth() === selectedMonth
      )
    );
  };

  const filteredCustomerData = filterDataByMonth(customerData, selectedMonth);
  ...
  const generateHTML = () => {
    ...
    const totalDebit = customerData.reduce((acc, customer) => {
      const debitAmount = customer.amount
    ...
    const totalCredit = customerData.reduce((acc, customer) => {
      const creditAmount = customer.amount
    ...
    customerData.forEach((customer) => {
      ...
      customer.amount.forEach((amount) => {
  ...
  const generatePDF = async () => {
    try {
      ...
      const fileName = `${'customersData'}_${timestamp}.pdf`;
  ...
  return (
    <>
      ...
      <View className="p-3" style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
        <TouchableOpacity onPress={() => navigation.navigate('Customer', {}, -1)}>
      ...
      <View className="mx-2 -mt-2 ">
        ...
        <FlatList
          ...
          data={customerData}
          renderItem={({ item }) => renderCustomerRows(item)}
...
export default CustomerReportByAccount